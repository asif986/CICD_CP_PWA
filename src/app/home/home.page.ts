...
import {ActionSheetController,AlertController,Events, IonSlides, LoadingController,ModalController,NavController, Platform} from '@ionic/angular';
...
export class HomePage implements OnInit {
  ...
  token_type_id_FromFilter: any;
    ...
    constructor(private storage: Storage, public actionSheetController: ActionSheetController, public loadingController: LoadingController, public alertController: AlertController, public helper: Helper, public apiservice: APIService, private network: Network,  private platform: Platform, private router: Router,private alertCtrl: AlertController,
                // tslint:disable-next-line:max-line-length
                private http: HttpClient, private uniqueDeviceID: UniqueDeviceID, public navCtrl: NavController, private dataService: DataService, private modalController: ModalController, public events: Events) {
      //  Unique Device ID
...
// 1==>Banner API ,2==>Feed API , 3==>Search API , 4==>DoInfinte Api , 5==>Do Refresh ,6==>RemoveorAddTeamLeader
  // Call Banner API
    getBannerData() {
      if (this.network.type !== 'none' && this.network.type !== 'unknown') {
                this.apiservice.GetAllProjectBanners(this.newLoginApi).subscribe(data => {
   ...
   getFeedData() {
    ...
    if (this.network.type !== 'none' && this.network.type !== 'unknown') {
           this.apiservice.getCPFeed(this.cpfeedData.cp_executive_id, this.cpfeedData.api_token, 0,null).subscribe(data => {
    ...
    if (title.length >= 1) {
      ...
      if (!(this.network.type !== 'none' && this.network.type !== 'unknown')) {
        this.helper.presentToast('Please on Internet Connection');
          ...
          this.apiservice.getCPFeedSeach(this.cpfeedData.cp_executive_id, this.cpfeedData.api_token, title, 0,null).subscribe(data => {
            me.isSpinner = false;
            this.successvalue = JSON.stringify(data.body);
            const Value = JSON.parse(this.successvalue);
            if (Value.success === 1) {
  ...
  if (this.searchVal != '') {
      ...
      if (title.length >= 1) {
          ...
          this.cpfeedData.api_token = this.newLoginApi;
          this.cpfeedData.cp_executive_id = this.newExecutiveId;
          this.apiservice.getCPFeedSeach(this.cpfeedData.cp_executive_id, this.cpfeedData.api_token, title, count,this.last_lead_updated_at).subscribe(data => {
              this.successvalue = JSON.stringify(data.body);
              const Value = JSON.parse(this.successvalue);
              if (Value.success === 1) {
      ...
      if (this.leadlist.length > 2) {
          const me = this;
          this.apiservice.getCPFeed(this.cpfeedData.cp_executive_id, this.cpfeedData.api_token, count,this.last_lead_updated_at).subscribe(data => {
              this.successvalue = JSON.stringify(data.body);
              const Value = JSON.parse(this.successvalue);
              if (Value.success === 1) {
                  if (Value.data.length != 0) {
  ...
  setTimeout(() => {
   ...
   if (this.network.type !== 'none' && this.network.type !== 'unknown') {
         this.apiservice.getCpTeamLeaderorNot( this.teamLeaderornot.api_token,this.teamLeaderornot.cp_executive_id).subscribe(data => {
               this.successvalue = JSON.stringify(data.body);
               const Value = JSON.parse(this.successvalue);
               console.log('successvalue', Value.success);
               this.isSpinner = false;
      ...
      if (val) {
        ...
        this.storage.get('apiToken').then((data) => {
          ...
          me.storage.get('fcmToken').then((fcm) => {
            if (fcm) {
              me.fcmToken = fcm;
              const param = {user_id: this.userId, device_id: this.uuid, device_type: 'android', fcm_token: me.fcmToken,
                api_token: this.apiToken};
              console.log(param);
              me.http.post('http://vjpartners.co.in/ongoing/v7_new/public/api/addAndUpdateFcmToken', param)
                  .subscribe(res => {
